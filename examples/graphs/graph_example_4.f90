...
implicit none
    ...
    nn = 2048
    d = ceiling(log(float(nn))/log(2.0))
...
20  format('Rewired all edges of ring graph with probability ',f8.6,'.')
    ...
    !----------------------------------------------------------------------!
    allocate(neighbors(g%max_degree), adj(g%max_degree,g%max_degree))
